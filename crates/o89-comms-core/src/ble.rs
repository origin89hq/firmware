...
mod vectors {
    ...
    fn p_036_p_037_p_039_shared_ble_vectors_exercise_the_radio_adapter_codecs() {
        ...
        let mut count = 0;
        for (line, text) in include_str!("../tests/ble-vectors.txt").lines().enumerate() {
            if text.starts_with('#') {
                continue;
            }
            count += 1;
            let mut fields = text.split('|');
            let action = fields.next().expect("action");
            let mtu = BleMtu::new(fields.next().expect("mtu").parse().expect("number"))
                .expect("valid mtu");
            let now = fields.next().expect("now").parse().expect("tick");
            let input_len = decode(fields.next().expect("input"), &mut input);
            let outcome = fields.next().expect("expected");
            let expected_len = decode(fields.next().expect("output"), &mut expected);
            let limit = fields.next().expect("limit");
            let limit = if limit.is_empty() {
                mtu.value_limit()
            } else {
                mtu.select(limit.parse().expect("limit number"))
                    .expect("limit")
            };
            let actual = match action {
                ...
                "fragment" | "small_buffer" => {
                    ...
                    match pipe.fragment(true, &mut fragment[..size]) {
                        Ok(len) => {
                            assert_eq!(&fragment[..len], &expected[..expected_len], "line {line}");
                            "ok"
                        }
                        Err(e) => error(e),
                    }
                ...
                "receive" => match pipe.receive(&input[..input_len], mtu, now) {
                    Ok(Some(bytes)) => {
                        assert_eq!(bytes, &expected[..expected_len], "line {line}");
                        "message"
                    }
                    Ok(None) => "pending",
                    Err(e) => error(e),
            ...
            assert_eq!(actual, outcome, "line {line}");
        }
        assert_eq!(count, 2758, "consume every published step");
    }
}

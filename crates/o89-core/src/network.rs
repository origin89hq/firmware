...
/// Two bytes of ISO 3166-1 alpha-2: ASCII capitals, nothing else.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
pub struct Country([u8; 2]);

/// Two bytes that are not a country code.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
pub struct BadCountry;

impl Country {
    /// `code`, refused unless both bytes are ASCII capitals.
    pub const fn new(code: [u8; 2]) -> Result<Self, BadCountry> {
...
mod tests {
    ...
    #[test]
    fn l_134_a_country_is_two_ascii_capitals() {

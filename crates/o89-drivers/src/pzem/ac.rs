...
//!
//! The two models share the table and differ in range, §1.2.1 and §1.3.1:
//! the PZEM-014 measures to 10 A and 2.3 kW, the PZEM-016 to 100 A and
//! 23 kW. A [`Meter`] is built for one [`Model`], and a reading past its
//! own model's range publishes `out_of_range`.
//!
...
/// The measurement registers, §2.3, with a model's current and power
/// ceilings from §1.2.1 and §1.3.1 and the ranges §1 gives both.
const fn cells(current_max: i32, power_max: i32) -> [Cell; 5] {
    [
        ...
        // 0x0001 and 0x0002, low word first, 0.001 A, from 0.
        Cell {
            register: 0x0001,
            kind: Kind::AcCurrent,
            domain: SignalDomain::Live,
        ...
        // 0x0003 and 0x0004, low word first, 0.1 W, from 0.
        Cell {
            register: 0x0003,
            kind: Kind::AcPower,
            domain: SignalDomain::Live,
...
/// The PZEM-014: to 10 A and 2.3 kW.
const CELLS_014: [Cell; 5] = cells(10_000, 23_000);
/// The PZEM-016: to 100 A and 23 kW.
const CELLS_016: [Cell; 5] = cells(100_000, 230_000);

/// The PZEM-014's whole measurement table in one read, §2.3's own example
/// request.
pub const PZEM_014: Block = crate::block!(Function::ReadInput, 0x0000, 10, &CELLS_014);
/// The PZEM-016's whole measurement table in one read, §2.3's own example
/// request.
pub const PZEM_016: Block = crate::block!(Function::ReadInput, 0x0000, 10, &CELLS_016);

/// Which of the two meters, and so which ranges hold.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
pub enum Model {
    /// The PZEM-014, with its built-in 10 A shunt.
    Pzem014,
    /// The PZEM-016, with its external 100 A transformer.
    Pzem016,
}

impl Model {
    /// The model's checked block.
    #[must_use]
    pub const fn block(self) -> &'static Block {
        match self {
            Self::Pzem014 => &PZEM_014,
            Self::Pzem016 => &PZEM_016,
        }
    }
}
...
impl Words {
    /// The words in `registers`, which must be a [`Model::block`]'s read.
    ///
...
impl Meter {
    /// How many signals a meter publishes, one per cell.
    pub const SIGNALS: usize = CELLS_016.len();

    /// The `model` at `address`, whose cells publish as consecutive signals
    /// from `first` in its block's order: voltage, current, power, energy,
    /// frequency.
    ///
    /// `None` when the signals would run past `0xFFFF`.
    #[must_use]
    pub fn new(address: Address, first: Id, model: Model) -> Option<Self> {
    ...
    /// The signal the `index`th cell publishes as, of [`Meter::SIGNALS`]
    /// from the first; `None` past the last.
    #[must_use]
...
mod tests {
    ...
    fn meter_of(model: Model) -> Meter {
        Meter::new(Address::new(1).unwrap(), id(50), model).unwrap()
    }

    fn meter() -> Meter {
        meter_of(Model::Pzem016)
    }
    ...
    fn poll(words: &[u16], store: &mut Signals<5>) -> Result<Report<Words>, VendorError<u8>> {
        let echo = PZEM_016.read(Address::new(1).unwrap()).encode();
    ...
    fn the_request_is_the_manual_s_example() {
        ...
        assert_eq!(
            PZEM_016.read(Address::new(1).unwrap()).encode(),
    ...
    fn a_meter_s_signals_are_consecutive_and_refused_past_the_top_of_the_range() {
        ...
        let at = Address::new(1).unwrap();
        assert!(Meter::new(at, id(0xFFFB), Model::Pzem014).is_some());
        assert_eq!(Meter::new(at, id(0xFFFC), Model::Pzem014), None);
    }

    #[test]
    fn p_185_each_model_s_current_and_power_ceiling_is_its_own() {
        // (model, current words, power words, current published, power
        // published), built by hand: the PZEM-014 reads to 10 A and 2.3 kW,
        // the PZEM-016 to 100 A and 23 kW (§1.2.1, §1.3.1).
        type Case = (Model, [u16; 2], [u16; 2], Option<i32>, Option<i32>);
        let cases: [Case; 5] = [
            (
                Model::Pzem014,
                [10_000, 0],
            ...
            (Model::Pzem014, [10_001, 0], [23_001, 0], None, None),
            (Model::Pzem014, [11_000, 0], [24_200, 0], None, None),
            (
                Model::Pzem016,
                [11_000, 0],
                [24_200, 0],
        ...
        for (model, current, power, amps, watts) in cases {
            let mut words = MANUAL;
            words[1..3].copy_from_slice(&current);
            words[3..5].copy_from_slice(&power);
            let echo = model.block().read(Address::new(1).unwrap()).encode();
            let (reply, len) = register_reply(1, 0x04, &words);
            let bursts: [&[u8]; 2] = [&echo, &reply[..len]];
            let mut port = Scripted::new(&bursts);
            let mut store = store();
            let report = block_on(meter_of(model).poll(&mut port, TIMING, Tick::ZERO, &mut store));
            assert_eq!(report.map(|report| report.polled.written), Ok(5));
            let [_, seen_amps, seen_watts, _, _] = published(&store);
            assert_eq!(
                (seen_amps.0, seen_watts.0),
                (amps, watts),
                "{model:?} {words:?}"
            );
            for (expected, signal) in [(amps, 51), (watts, 52)] {
                if expected.is_none() {
                    let sample = store.sample(id(signal), Tick::ZERO).unwrap();
                    assert_eq!(sample.q.validity_of(), Validity::OutOfRange);
                }
            }
        }
        assert_eq!(
            Model::Pzem014.block().read(Address::new(1).unwrap()),
            PZEM_016.read(Address::new(1).unwrap())
        );
    }
}

//! The monotonic tick every duration is measured on.
//!
...
impl Millis {
    ...
    /// The whole seconds in the duration, the fraction dropped.
    #[must_use]
    pub const fn as_secs(self) -> u64 {
        match self.0.checked_div(1_000) {
            Some(secs) => secs,
            None => 0,
        }
    }
...
mod tests {
    ...
    #[test]
    fn a_duration_counts_whole_seconds_and_drops_the_fraction() {
        assert_eq!(Millis::from_millis(3_999).as_secs(), 3);
        assert_eq!(Millis::from_millis(999).as_secs(), 0);
        assert_eq!(
            Millis::from_millis(u64::MAX).as_secs(),

...
| H5 | A hung task leaves an output where it was while the watchdog is fed by a timer | The watchdog is fed only when every task has checked in inside its period | The rollcall supervisor (F-007); the blame that survives the reset (F-008) | M1: deliberate starvation, the reset, the next boot naming the task | none yet | — |
...
| H15 | A firmware change locks the probe out of a revision A board for good (hardware#29) | No stop mode, no SWD pin reuse on revision A | F-012 | Review, and the board revision as a build-time type that makes the stop-mode path unreachable on revision A | none yet | — |

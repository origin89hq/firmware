...
| H5 | A hung task leaves an output where it was while the watchdog is fed by a timer | The watchdog is fed only when every task has checked in inside its period | The rollcall supervisor (F-007); the blame that survives the reset (F-008) | M1: deliberate starvation, the reset, the next boot naming the task | host: `f_007_*` and `f_008_*` in `o89-core` run the rollcall and the last words on a laptop, 2026-09-18; bench 2026-09-18: the proof chain on board A revision A, three records naming the task and then the panic site (`docs/bench/2026-09-18.md`) | — |

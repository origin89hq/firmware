...
and size before code lands. Nothing runs on a board yet. The design is
...
| `crates/o89-drivers` | The controller's device dialects behind port seams: Modbus RTU framing, the register maps with each cell's declared provenance, the Pylontech-class CAN frames, and the device enum; `no_std`, no allocator, names no peripheral |
